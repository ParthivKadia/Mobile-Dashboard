const TopAppBar = ({ setActivePage }) => {
  return (
    <header className="fixed top-0 w-full z-50 bg-[#f7f9fb]/80 backdrop-blur-xl shadow-[0px_20px_40px_rgba(0,80,203,0.06)] px-6 py-4">
      <div className="flex justify-between items-center w-full max-w-screen-xl mx-auto">
        <div className="flex items-center gap-4">
          <button
            onClick={() => setActivePage('store-profile')}
            className="h-10 w-10 rounded-full bg-surface-container overflow-hidden active:scale-95 duration-200 transition-all"
          >
            <img
